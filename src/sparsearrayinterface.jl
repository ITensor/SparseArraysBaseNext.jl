...
function Base.getindex(::SparseArrayInterface, a, I::Int...)
  !isstored(a, I...) && return getunstoredindex(a, I...)
  return getstoredindex(a, I...)
end

function Base.setindex!(::SparseArrayInterface, a, value, I::Int...)
...
function Base.similar(::SparseArrayInterface, a, T::Type, size::Tuple{Vararg{Int}})
...
## function Base.similar(interface::SparseArrayInterface, a, T::Type, dims::Tuple{Vararg{Base.OneTo}})
##   return similar(interface, a, T, Base.to_shape(dims))
## end

function Base.map(::SparseArrayInterface, f, as...)
...
function Base.map!(::SparseArrayInterface, f, dest, as...)
...
Base.BroadcastStyle(::SparseArrayInterface, type::Type) = SparseArrayStyle{ndims(type)}()
...
function Base.similar(bc::Broadcast.Broadcasted{<:SparseArrayStyle}, T::Type, axes::Tuple)
  ...
  return similar(SparseArrayInterface(), bc, T, Int.(length.(axes)))
end
...
function Base.copyto!(dest::AbstractArray, bc::Broadcast.Broadcasted{<:SparseArrayStyle})
  map!(SparseArrayInterface(), map_function(bc), dest, map_args(bc)...)
  return dest
end
